...
import { Check, ArrowRight, ArrowLeft, Star, Calendar, Clock, User, MapPin, FileText } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import Layout from "@/components/layout/Layout";
import { services, Service } from "@/data/services";
import { nurses, Nurse } from "@/data/nurses";
...
export default function BookNurse() {
  ...
  const handleConfirm = () => {
    ...
    const id = `ICH-${Date.now().toString(36).toUpperCase()}`;
    setBookingId(id);
    setCurrentStep(5);
  };
  ...
  const isStepValid = () => {
    switch (currentStep) {
      ...
      case 4:
        return formData.fullName && formData.phone && formData.email && formData.address;
      default:
        return true;
    }
  };
          ...
          <div className="max-w-4xl mx-auto">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {services.map((service) => (
                    <button
                      ...
                      className={cn(
                        "card-medical text-left p-5 cursor-pointer border-2 transition-all",
            ...
            {currentStep === 2 && (
              <div className="animate-fade-up">
                ...
                <p className="text-muted-foreground text-center mb-8">
                  Select from our team of qualified professionals
                </p>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {nurses.map((nurse) => (
                    <button
                      ...
                      className={cn(
                        "card-medical text-left p-5 cursor-pointer border-2 transition-all relative",
                    ...
                    >
                      {!nurse.available && (
                        <span className="absolute top-3 right-3 px-2 py-1 bg-muted text-muted-foreground text-xs rounded-full">
                          Unavailable
                        </span>
                      )}
                      <div className="flex flex-col items-center text-center">
                        ...
                        <div className="flex flex-wrap gap-1 justify-center">
                          {nurse.specialty.map((spec, i) => (
                            <span
                              key={i}
                              className="px-2 py-0.5 bg-accent text-secondary text-xs rounded-full"
                            >
                              {spec}
                            </span>
                          ))}
            ...
            {currentStep === 3 && (
              <div className="animate-fade-up">
                ...
                <p className="text-muted-foreground text-center mb-8">
                  Select when you'd like the appointment
                </p>
                      ...
                      {generateDates().map((date) => {
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              "flex flex-col items-center px-4 py-3 rounded-xl border-2 min-w-[80px] transition-all",
                ...
                <div className="card-medical max-w-lg mx-auto text-left">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between py-3 border-b border-border">
                      ...
                      <span className="font-medium text-foreground">{selectedNurse?.name}</span>
                    </div>
                    <div className="flex justify-between py-3 border-b border-border">
